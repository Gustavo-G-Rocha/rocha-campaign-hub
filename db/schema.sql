...
-- `seed_log`. Sem isso, tudo o que a equipe apagasse pelo painel (/admin)
-- voltaria no próximo start do servidor, já que este arquivo é reaplicado
-- a cada deploy.
-- ------------------------------------------------------------
...
DO $seed$
BEGIN
  IF EXISTS (SELECT 1 FROM seed_log WHERE chave = 'dados-iniciais-v1') THEN
    RETURN;
  END IF;
